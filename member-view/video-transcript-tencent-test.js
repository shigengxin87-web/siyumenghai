...
const rawTranscript=document.querySelector('#raw-transcript-text');
const formatCheck=document.querySelector('#format-check');
...
function renderJob(job){card.classList.remove('hidden');currentJobId=job.id;setStatus(job.status,job.stage);message.textContent=job.error||(['queued','processing'].includes(job.status)?'任务正在后台继续处理，可以刷新页面或查看其他视频。':'');if(job.video_duration_seconds)showMetrics(job);else metrics.classList.add('hidden');if(job.status==='completed'&&job.text){const formatted=formatTranscript(job.text);rawTranscript.value=job.text;transcript.value=formatted.formatted;formatCheck.textContent=formatted.verified?'仅增加段落，原文校验通过':'排版校验未通过，已显示原始稿';formatCheck.classList.toggle('fallback',!formatted.verified);resultWrap.classList.remove('hidden')}else{transcript.value='';rawTranscript.value='';resultWrap.classList.add('hidden')}upsertHistory({jobId:job.id,shareUrl:job.share_url,status:job.status,stage:job.stage,elapsed:job.elapsed_seconds,chars:job.char_count});loadPlayer(job.share_url)}
...
document.querySelector('#copy-button').onclick=async()=>{await navigator.clipboard.writeText(transcript.value);message.textContent='排版逐字稿已复制。'};
document.querySelector('#copy-raw-button').onclick=async()=>{await navigator.clipboard.writeText(rawTranscript.value);message.textContent='腾讯原始识别稿已复制。'};
