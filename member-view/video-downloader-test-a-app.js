...
const TEST_VARIANT = String(document.documentElement.dataset.transcriptVariant || '').toLowerCase();
const TEST_CONFIG = Object.freeze({
  a: Object.freeze({ transcriptApi: 'http://127.0.0.1:3015/jobs' }),
  b: Object.freeze({ transcriptApi: 'http://127.0.0.1:8768/jobs' }),
  c: Object.freeze({ transcriptApi: '/api/transcripts-test-cloud/jobs' })
});
if (!TEST_CONFIG[TEST_VARIANT]) throw new Error('缺少逐字稿测试线路配置');
...
const TEST_STORAGE_PREFIX = `siyumenghai-video-test-${TEST_VARIANT}`;
const HISTORY_KEY = `${TEST_STORAGE_PREFIX}-download-history-v1`;
const HISTORY_LIMIT = 20;
const TRANSCRIPT_CACHE_KEY = `${TEST_STORAGE_PREFIX}-transcripts-v2-bound-response`;
const TRANSCRIPT_CACHE_LIMIT = HISTORY_LIMIT;
const TRANSCRIPT_TASK_KEY = `${TEST_STORAGE_PREFIX}-transcript-tasks-v2-bound-response`;
const TRANSCRIPT_TASK_LIMIT = HISTORY_LIMIT;
const COMMENT_CACHE_KEY = `${TEST_STORAGE_PREFIX}-comments-v1`;
const COMMENT_CACHE_LIMIT = HISTORY_LIMIT;
const TRANSCRIPT_API = TEST_CONFIG[TEST_VARIANT].transcriptApi;
...
function transcriptElapsedText(seconds) {
  const value = Number(seconds || 0);
  if (!(value > 0)) return '';
  return value < 60 ? `${Math.ceil(value)} 秒` : `${Math.floor(value / 60)} 分 ${Math.ceil(value % 60)} 秒`;
}
...
function renderHistory() {
  ...
  items.forEach((item, index) => {
    ...
    if (transcript) {
      const elapsedText = transcriptElapsedText(transcript.elapsed);
      taskStatus.textContent = `逐字稿已完成${elapsedText ? ` · 用时 ${elapsedText}` : ''}`;
...
function renderResult(payload, shareUrl) {
  ...
  if (previousTranscript) {
    ...
    const elapsedText = transcriptElapsedText(previousTranscript.elapsed);
    showTranscriptStatus(`已读取本机缓存的逐字稿${elapsedText ? `，本次生成用时 ${elapsedText}` : ''}${previousTranscript.correctionCount ? `，其中校正 ${previousTranscript.correctionCount} 处` : ''}。`);
  }
